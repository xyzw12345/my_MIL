...
-- ID: 355 (from 300 Problems in Abstract Algebras)

#check BooleanRing

#check BooleanRing.toCommRing

instance (α : Type*) : BooleanAlgebra (Set α) := Set.instBooleanAlgebraSet

instance (α : Type*) : BooleanRing (Set α) := BooleanAlgebra.toBooleanRing

-- The universal set is the identity element.
example (α : Type*) : @One.one (Set α) _ = @Set.univ α := rfl



section

-- ID: 116 (from 300 Problems in Abstract Algebras)
variable {G : Type*}

def inv_monoidHom_of_comm [CommGroup G] : MonoidHom G G where
  toFun := fun g ↦ g⁻¹
  map_one' := by
    simp only [inv_one]
  map_mul' := by
    intro a b
    dsimp
    have : (a * b)⁻¹ = b⁻¹ * a⁻¹ := mul_inv_rev a b
    rw [mul_inv_rev a b]
    exact CommGroup.mul_comm b⁻¹ a⁻¹

example [hg : Group G] (f : MonoidHom G G) (h : ∀ g : G, f g = g⁻¹) : CommGroup G := {
  hg with
  mul_comm := by
    intro a b
    have hab := map_mul f a⁻¹ b⁻¹
    rw [h, h, h] at hab
    simp only [mul_inv_rev, inv_inv] at hab
    exact hab.symm}
