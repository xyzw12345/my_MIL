...
def Submonoid.Setoid [CommMonoid M] (N : Submonoid M) : Setoid M  where
  ...
  iseqv := {
    ...
    symm := by
      ...
      rcases c with ⟨d, d_in_N, f, f_in_N, eq⟩
      -- From `eq` you should be able to know what to `use`.
      use f
      constructor
      exact f_in_N
      use d
      exact ⟨d_in_N, eq.symm⟩
    ...
    trans := by
      ...
      rw [← mul_assoc, eq₁, ← mul_assoc, mul_comm a, mul_assoc c a e, ← eq₂, ← mul_assoc, mul_comm c]
  }
...
theorem MyLeftCosetEquivalence_rel (s : Set α) : Equivalence (LeftCosetEquivalence s) := by
  ...
  intro _ _ z; exact z.symm
  -- `trans` part of the proof.
  intro _ _ _ h₁ h₂; rw [h₁, h₂]
...
theorem MyRightCosetEquivalence_rel (s : Set α) : Equivalence (RightCosetEquivalence s) := by
  unfold RightCosetEquivalence
  constructor
  exact (fun _ ↦ (by rfl))
  intro _ _ z; exact z.symm
  intro _ _ _ h₁ h₂; rw [h₁, h₂]
...
theorem practice₂: ∀ (a : G), ∃ (b c : G), (b • ((H₁: Subgroup G) : Set G)) ∩ (c • ((H₂: Subgroup G) : Set G)) = a • (H₁ ∩ H₂: Set G) := by
  ...
  exact (mem_leftCoset_iff d).mpr ⟨((mem_leftCoset_iff d).mp h₁), ((mem_leftCoset_iff d).mp h₂)⟩
  -- With the experience above, the opposite direction should not be difficult.
  intro x y
  have x_union: d⁻¹ * x ∈ (H₁ ∩ H₂ : Set G) := (mem_leftCoset_iff d).mp y
  rcases x_union with ⟨x₁, x₂⟩
  exact ⟨((mem_leftCoset_iff d).mpr x₁), ((mem_leftCoset_iff d).mpr x₂)⟩
...
instance [CommMonoid M] (N : Submonoid M) : Monoid (M ⧸ N) where
  ...
  mul := Quotient.map₂' (· * ·) (by
    ...
    apply N.mul_mem hw hw'
    -- Follow the same logic, we can get a simplified equation.
    use z * z'
    constructor
    apply N.mul_mem hz hz'
    -- A shorter proof: refine ⟨w * w', N.mul_mem hw hw', z * z', N.mul_mem hz hz', ?_⟩
    -- Practise your patience now playing with `rw`.
    rw [← mul_assoc, ← mul_assoc, mul_comm, ← mul_assoc, ← mul_assoc, mul_comm, mul_comm w', ← mul_assoc, ← mul_assoc, mul_comm w, ha, mul_assoc, mul_comm w', hb, ← mul_assoc, mul_comm, mul_comm b₁, mul_assoc, ← mul_assoc, mul_comm, mul_comm z', ← mul_assoc])
  mul_assoc := by
    rintro ⟨a⟩ ⟨b⟩ ⟨c⟩
...
theorem practice₅ {N : Subgroup G} [nN : N.Normal] (x : G) : (x : G ⧸ N) = 1 ↔ x ∈ N := by
  ...
  have: 1⁻¹ * x ∈ N := (practice₄ N).mp h
  simp only [inv_one, one_mul] at this
  exact this
  ...
  have: 1⁻¹ * x ∈ N := by
    ...
    exact h
  apply (practice₄ N).mpr
  exact this

-- From the statements above we can conclude the kernel of the group homomorphism from `G` to `G / N` is `N`.
theorem practice₆ : MonoidHom.ker (QuotientGroup.mk' N : G →* G ⧸ N) = N := Subgroup.ext eq_one_iff
